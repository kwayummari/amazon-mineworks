...
// For now, we'll use a mock database service since we can't directly connect to MySQL from the frontend
// In a real application, you would have a backend API that handles database operations
...
class DatabaseService {
            ...
            admins: [
                {
                    id: 1,
                    username: 'admin',
                    password: 'z$T8zb|>GXA7', // This should be hashed in production
                    email: 'admin@amazonmineworks.com',
                    role: 'administrator',
                ...
                },
                {
                    ...
                    password: ';@DI$>yv3Zc"bi1v', // This should be hashed in production
    ...
    async getNews() {
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                // If no local news, initialize with default data
                if (localNews.length === 0) {
                    localStorage.setItem('news', JSON.stringify(this.mockData.news));
                    resolve(this.mockData.news);
                } else {
                    resolve(localNews);
                }
            }, 500);
    ...
    async resetNewsToDefault() {
        return new Promise((resolve) => {
            setTimeout(() => {
                localStorage.setItem('news', JSON.stringify(this.mockData.news));
                console.log('News data reset:', this.mockData.news); // Debug log
                resolve({ success: true, message: 'News reset to default data' });
            }, 500);
        });
    }
    ...
    async debugNewsData() {
        console.log('Mock data:', this.mockData.news);
        console.log('LocalStorage data:', JSON.parse(localStorage.getItem('news') || '[]'));
        console.log('All localStorage keys:', Object.keys(localStorage));
    }

    // Force initialize with default data
    async forceInitializeNews() {
        localStorage.setItem('news', JSON.stringify(this.mockData.news));
        console.log('Force initialized news data:', this.mockData.news);
        return { success: true, data: this.mockData.news };
    }
